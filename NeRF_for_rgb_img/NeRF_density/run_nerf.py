...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    #####################################3
    uncert_density = raw[...,4] # ocupancy이지만 일단 softplus 적용되어 있음. 일단 이렇게하고 나중에 수정해보자.
    uncert_density = torch.clamp(uncert_density, min=1e-9, max=200.)
    print("uncert_density(max:200): ", torch.mean(uncert_density))
    print("uncert_den min: ", torch.min(uncert_density))
    print("uncert_den max: ", torch.max(uncert_density))
    ...
    ocupancy = 1-torch.exp(-distXdensity)
    print("ocupancy: ", torch.mean(ocupancy))

    ###############################
    Ti_copy = Ti.unsqueeze(-1).expand(-1, -1, 3)
    dists_copy = dists.unsqueeze(-1).expand(-1, -1, 3)
    raw_3_copy = raw[..., 3].unsqueeze(-1).expand(-1, -1, 3)
    raw_4_copy = raw[..., 4].unsqueeze(-1).expand(-1, -1, 3)

    # rgb_map = torch.sum(weights[...,None] * rgb, -2)  # [N_rays, 3]
    rgb_map = torch.sum(rgb * dists_copy * raw_3_copy, -2)  
    uncert_map = torch.sum(rgb * rgb * dists_copy * dists_copy * raw_4_copy, -2) # [ray * sample]k

    print("Ti: ", torch.mean(Ti))
    print("raw[..., 4]: ", torch.mean(Ti))
    print("uncert_map: ", torch.mean(uncert_map))
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        loss_imguncert = img2mse_uncert_alpha(rgb, target_s, uncert, alpha, args.w)
        #############################################################################################################
        if i < 1000: # pretraining
            img_loss = loss_imgmse
        elif 1000<=i and i<1200:
        ...
        loss = img_loss
        psnr = txt_psnr
        print("pred rgb: ", torch.mean(rgb))
        print("gt rgb: ", torch.mean(target_s))
        print("loss_imgmse :", loss_imgmse)
        print("loss uncert: ", loss_imguncert)
        print("loss: ", img_loss)
        print("psnr: ", psnr)
        ...
        if i%args.i_video==0 and i > 0:
            ...
            with torch.no_grad():
                rgbs, disps, _, _ = render_path(render_poses, hwf, K, args.chunk, render_kwargs_test)
