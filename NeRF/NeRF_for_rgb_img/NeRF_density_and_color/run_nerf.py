...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    # print("minjae rgb",rgb)

    # rgb_mean = rgb.mean(dim=-1)
    density_copy = F.relu(raw[...,3]).unsqueeze(-1).expand(-1, -1, 3) # Den
    dists_copy = dists.unsqueeze(-1).expand(-1, -1, 3) # dist
    raw_3_copy = raw[..., 3].unsqueeze(-1).expand(-1, -1, 3) #den
    raw_4_copy = raw[..., 4].unsqueeze(-1).expand(-1, -1, 3) # U den
    raw_5_copy = raw[..., 5].unsqueeze(-1).expand(-1, -1, 3) # U color

    #################################################################################
    rgb_map = torch.sum(weights[...,None] * rgb, -2)  # [N_rays, 3]
    # rgb_map = torch.sum(dists_copy * raw_3_copy * rgb, -2)  # [N_rays, 3]
    ################################################################################
    uncert_map = torch.sum(dists_copy * dists_copy *(raw_4_copy * rgb + raw_5_copy * density_copy + raw_5_copy * raw_4_copy), -2)


    print("uncert_map mean: ", torch.mean(uncert_map))
    print("uncert_map max: ", torch.max(uncert_map))
    print("uncert_map min: ", torch.min(uncert_map))
    # print("weight mean: ", weights.mean())
    # print("alpha mean: ", alpha.mean())
    # print("density mean: ", raw[...,3].mean())
    # print("density relu mean: ", F.relu(raw[...,3]).mean())
    ##################################################################################
...
def config_parser():
    ...
    parser.add_argument("--testskip", type=int, default=2, #2
                        help='will load 1/N images from test/val sets, useful for large datasets like deepvoxels')
    ########################################################################################################   
