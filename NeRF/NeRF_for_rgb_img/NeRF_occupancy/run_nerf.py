...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    #####################################3
    uncertainty_ocupancy = raw[...,4] # ocupancy이지만 일단 softplus 적용되어 있음. 일단 이렇게하고 나중에 수정해보자.
    uncertainty_ocupancy = torch.clamp(uncertainty_ocupancy, min=1e-9, max=200.)
    print("uncertainty_ocupancy(max:200): ", torch.mean(uncertainty_ocupancy))
    print("uncert_ocu min: ", torch.min(uncertainty_ocupancy))
    print("uncert_ocu max: ", torch.max(uncertainty_ocupancy))
    ...
    # uncert_map = torch.sum(rgb.detach() * rgb.detach() * dists_copy * dists_copy * raw_4_copy , -2) # [ray * sample]k
    uncert_map = torch.sum(Ti_copy * Ti_copy * rgb * rgb * raw_4_copy, -2) # [ray * sample]k
