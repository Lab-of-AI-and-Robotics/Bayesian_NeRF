...
def render_path_squence(c2w, up, rad, focal,  poses, N):
    render_poses = []
    hwf = c2w[:,4:5]
    ...
    for t in np.linspace(0.,1.,N+1)[:-1]:
        c = pos_sta + t * (pos_end - pos_sta)
        z = z0 + t * (z1 - z0)
        i += 1
        # z = normalize(-np.random.rand(1) * v * 4 + focal * c2w[:,2])
        render_poses.append(np.concatenate([viewmatrix(z, up, c), hwf], 1))
    return render_poses
...
def load_llff_data(basedir, factor=8, recenter=True, bd_factor=.75, spherify=False, path_zflat=False):
    ...
    print('bds after *sc:', bds.min(), bds.max())
    ...
    if spherify:
        ...
        print('bds after spherify:', bds.min(), bds.max())
        ...
        # generate poses near train poses
        # render_poses = 

        # Generate poses for sequence path
        # render_poses = render_path_squence(c2w, up, rads, focal, poses, poses.shape[0])
        # up = normalize(poses[22:29, :3, 1].sum(0))
        # render_poses = render_path_squence(c2w, up, rads, focal, poses, 120)
