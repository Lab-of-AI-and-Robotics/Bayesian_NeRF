...
def evaluate_ate(first_list, second_list, plot="", txt_file=None, _args=""):
    ...
    output = (
        f"compared_pose_pairs {results['compared_pose_pairs']} pairs\n"
        f"absolute_translational_error.rmse {results['absolute_translational_error.rmse']} m\n"
        f"absolute_translational_error.mean {results['absolute_translational_error.mean']} m\n"
        f"absolute_translational_error.median {results['absolute_translational_error.median']} m\n"
        f"absolute_translational_error.std {results['absolute_translational_error.std']} m\n"
        f"absolute_translational_error.min {results['absolute_translational_error.min']} m\n"
        f"absolute_translational_error.max {results['absolute_translational_error.max']} m\n"
    )
...
def evaluate(poses_gt, poses_est, plot, txt_file=None):
    ...
    results = evaluate_ate(poses_gt, poses_est, plot, txt_file=txt_file)
    print(results)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--txt_name', type=str, help='storing information file path')
    ...
    if os.path.exists(ckptsdir):
        ...
        if len(ckpts) > 0:
            ...
            evaluate(poses_gt, poses_est,
                     plot=f'{output}/eval_ate_plot.png', txt_file=args.txt_name)
