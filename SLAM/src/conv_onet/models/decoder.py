...
class DenseLayer_uncert(nn.Linear):
    def __init__(self, in_dim: int, out_dim: int, activation: str = "relu", *args, **kwargs) -> None:
        self.activation = activation
        super().__init__(in_dim, out_dim, *args, **kwargs)
...
class MLP(nn.Module):
    ...
    def __init__(self, name='', dim=3, c_dim=128,
                 ...
                 color=False, skips=[2], grid_len=0.16, pos_embedding_method='fourier', concat_feature=False, uncert=False):
        super().__init__()
        self.name = name
        self.color = color
        ...
        if self.color:
            if self.uncert:
                self.output_linear = DenseLayer(
                    hidden_size, 5, activation="linear")
            else:
                self.output_linear = DenseLayer(
                    hidden_size, 4, activation="linear")
        else:
...
class NICE(nn.Module):
    ...
    def __init__(self, dim=3, c_dim=32,
                 ...
                 color_grid_len=0.16, hidden_size=32, coarse=False, pos_embedding_method='fourier', uncert=False):
        super().__init__()
        self.uncert = uncert
        if coarse:
        ...
        self.middle_decoder = MLP(name='middle', dim=dim, c_dim=c_dim, color=False,
                                  ...
                                  grid_len=middle_grid_len, pos_embedding_method=pos_embedding_method, uncert=self.uncert)
        self.fine_decoder = MLP(name='fine', dim=dim, c_dim=c_dim*2, color=False,
                                skips=[2], n_blocks=5, hidden_size=hidden_size,
                                grid_len=fine_grid_len, concat_feature=True, pos_embedding_method=pos_embedding_method, uncert=self.uncert)
        
        self.color_decoder = MLP(name='color', dim=dim, c_dim=c_dim, color=True,
                                 skips=[2], n_blocks=5, hidden_size=hidden_size,
                                 grid_len=color_grid_len, pos_embedding_method=pos_embedding_method, uncert=self.uncert)
        
        
        
        
        print(self.color_decoder)
    ...
    def forward(self, p, c_grid, stage='middle', **kwargs):
        ...
        if stage == 'coarse':
            ...
            occ = occ.squeeze(0)
            if self.uncert:
                raw = torch.zeros(occ.shape[0], 5).to(device).float()
                raw[..., -1] = occ
            else:
            ...
            if self.uncert:
                raw = torch.zeros(middle_occ.shape[0], 5).to(device).float()
                raw[..., -1] = middle_occ
            else:
            ...
            if self.uncert:
                raw = torch.zeros(fine_occ.shape[0], 5).to(device).float()
                middle_occ = self.middle_decoder(p, c_grid)
                middle_occ = middle_occ.squeeze(0)
            ...
            # 원래 raw : sample point들의 특성 -> rgb, occupancy
            # raw.shape = (9600, 4)
            
            # 수정 후 : rgb, uncertainty, occupancy
            # raw.shape = (9600, 5)
            # print(raw.shape)
            return raw
